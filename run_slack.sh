...
# Inisialisasi variabel global agar tidak unbound
declare -A JOB_DATA
...
load_input_csv() {
    ...
    while IFS=',' read -r e_order _rho e_ori_id _size _fill_a _fill_b \
...
run_scenario_inner() {
    ...
    for i in $(seq 0 $(( total - 1 ))); do
        ...
        if [[ -z "$job_info" ]]; then
            T_ORDER[$i]="${E_ORDER[$i]}"; T_ORI_ID[$i]=$ori_id
            T_SIZE[$i]="N/A"; T_FILL_A[$i]="N/A"; T_FILL_B[$i]="N/A"
            T_JOB_NAME[$i]="UNKNOWN"; T_POD_NAME[$i]="N/A"; T_ARRIVAL[$i]=$arrival_iso
        ...
        sed -e "s|<job_name>|${new_job_name}|g" -e "s|<max_runtime>|${max_runtime}|g" \
            -e "s|<cpu_usage>|${cpu_usage}|g" -e "s|<size>|${size}|g" \
            -e "s|<fill_a>|${fill_a}|g" -e "s|<fill_b>|${fill_b}|g" \
        ...
        T_ORDER[$i]="${E_ORDER[$i]}"; T_ORI_ID[$i]=$ori_id; T_SIZE[$i]=$size
        T_FILL_A[$i]=$fill_a; T_FILL_B[$i]=$fill_b; T_JOB_NAME[$i]=$new_job_name
        T_POD_NAME[$i]=$pod_name; T_ARRIVAL[$i]=$arrival_iso
    ...
    for i in $(seq 0 $(( total - 1 ))); do
        ...
        local status="MISSING" pod_creation="N/A" container_creation_ts="N/A" container_started="N/A"
        local finished="N/A" scheduled="N/A" queue_wait="N/A" deadline="N/A"
        ...
        echo "${T_ORDER[$i]},${rho_label},${T_ORI_ID[$i]},${T_SIZE[$i]},${T_FILL_A[$i]},${T_FILL_B[$i]},${T_JOB_NAME[$i]},${pod_name_metrics},${T_ARRIVAL[$i]},${pod_creation},${container_creation_ts},${container_started},${finished},${scheduled},${queue_wait},${deadline}" >> "$out_csv"
    done
    rm -f /tmp/metrics_slack_*.txt
}
...
run_edf_scenario() {
    ...
    [[ ! -f "$input_csv" ]] && return 1
    ensure_csv "$out_csv"
    load_input_csv "$input_csv" > /dev/null
